...
class PaymentCreateView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        serializer = serializers.BasketItemDataSerializer(
            data=request.data,
            many=True,
        )
        if serializer.is_valid():
            order = Order.objects.create(
                user=request.user.id, status="waiting_for_data"
            )
            for item in serializer.data:
                goods_data = item.pop("goods")
                ...
                session_data["line_items"].append(
                    {
                        "price_data": {
                            ...
                            "currency": "rub",
                            "product_data": {"name": item.goods_title},
                        },
...
class PaymentSuccessView(GenericAPIView):
    @extend_schema(
        description="В случае успешной оплаты платёжный сервис осуществит "
            ...
            200: OpenApiResponse(
                ...
                examples=[
                    OpenApiExample(
                        ...
                        value={"detail": "Оплата проведена успешно"},
                    )
                ],
            )
        },
...
class PaymentCanceledView(GenericAPIView):
    ...
    @extend_schema(
        description="В случае неудачной оплаты платёжный сервис осуществит "
        "redirect на этот url",
        responses={
            200: OpenApiResponse(
                inline_serializer(
                ...
                examples=[
                    OpenApiExample(
                        ...
                        value={"detail": "Оплата не проведена"},
                    )
                ],
            )
        },
