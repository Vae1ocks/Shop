...
def goods_create(
    category=None, title='Test Goods', amount=1,
    description='This is test goods', price=100, **kwargs
) -> Goods:
    if not category:
        category = Category.objects.first()
    return Goods.objects.create(
        category=category, title=title, amount=amount,
        description=description, price=price, **kwargs
    )
...
class FavoritesAPITest(APITestCase):
    ...
    def setUp(cls):
        ...
        cls.user = user_create(username=cls.username, password=cls.password)
        category1 = category_create(title='First Category')
        category2 = category_create(title='Second Category')
        goods = goods_create(category=category1)
        goods = goods_create(category=category1, title='Scnd Test Goods')
    ...
    def test_favorites_list(self):
        ...
        serializer = FavoriteSerializer([favorite1, favorite2], many=True)
    ...
    def test_favorite_delete(self):
        ...
        self.assertFalse(
            queryset.filter(user_id=user.id).exists()
        )
    ...
    def test_favorite_delete_forbidden(self):
        ...
        url = reverse(
            'favorites:delete', args=(fake_users_favorite1.id,)
        )
