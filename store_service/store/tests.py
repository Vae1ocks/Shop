...
class StoreTest(APITestCase):
    ...
    def setup(self, auth_required=True):
        ...
        self.goods4 = Goods.objects.create(
            title="Fourth Goods",
            price="10",
            amount=1,
    ...
    def test_goods_list_no_filter(self, mock_get):
        ...
        self.assertEqual(
            rec_data,
            GoodsListSerializer(
                [self.goods3, self.goods2, self.goods],
                many=True,
            ).data,
        )
        self.assertEqual(
            oth_data,
            GoodsListSerializer([self.goods4], many=True).data,
        )

    def test_goods_list_category_filter(self):
        self.setup()
        url = reverse(
            "store:goods_category_list",
            args=(self.category.title,),
        )
        ...
        self.assertEqual(
            data[0],
            GoodsListSerializer(self.goods2).data,
        )
        self.assertEqual(
            data[1],
            GoodsListSerializer(self.goods).data,
        )
    ...
    def test_goods_list_title_filter(self, mock_get):
        ...
        self.assertEqual(
            rec_data,
            GoodsListSerializer([self.goods3], many=True).data,
        )
        self.assertEqual(
            oth_data,
            GoodsListSerializer([self.goods2], many=True).data,
        )
    ...
    def test_goods_detail(self):
        ...
        self.assertEqual(
            response.json(),
            GoodsDetailSerializer(self.goods).data,
        )
    ...
    def test_goods_price_history(self):
        ...
        self.assertEqual(
            len(GoodsDetailSerializer(self.goods).data["price_history"]),
            2,
        )
    ...
    @patch("store.views.store.views.requests.get")
    def test_comment_create_with_no_user_personal_data(self, mock_get):
        self.setup()
        url = reverse("store:comment_create", args=[self.goods.id])
        data = {
            "body": "Some comment body",
    ...
    def test_comment_partial_update(self):
        ...
        data = {"body": "Some other comment body", "rating": 3}
