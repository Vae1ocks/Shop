...
class GoodsListView(GenericAPIView):
    ...
    @extend_schema(
        parameters=[
            ...
            OpenApiParameter(
                name="price",
                description="Фильтрация по цене доступна "
    ...
    def get(self, request, *args, **kwargs):
        ...
        if title or category:
            ...
            allowable_price = self.request.query_params.get(
                "price",
                None,
            )
            if allowable_price:
        ...
        if not category:
            ...
            other_goods = queryset.exclude(
                id__in=[goods.id for goods in recommended_goods]
            ).order_by("-times_bought")

            recommended_goods_data = GoodsListSerializer(
                recommended_goods, many=True
            ).data
            other_goods_data = GoodsListSerializer(other_goods, many=True).data
...
class CommentCreateView(CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if response.status_code == status.HTTP_200_OK:
            response_data = (
                response.json()
            )  # {'first_name': 'str', 'profile_picture': 'img'}
            user_data = {
                "author_name": response_data.get("first_name"),
                "author_profile_picture": response_data.get("profile_picture"),
            }
...
class CommentUpdateDeleteView(mixins.DestroyModelMixin, GenericAPIView):
