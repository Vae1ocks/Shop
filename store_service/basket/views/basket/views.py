...
class BasketAddGoodsView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            basket.add(goods, serializer.data['amount'], serializer.data['override'])
...
class BasketRemoveGoodsView(GenericAPIView):
    @extend_schema(
        ...
        request=inline_serializer(
            name='Remove Data',
            fields={'goods_id': serializers.PrimaryKeyRelatedField(queryset=Goods.objects.all())}
        ),
        responses={
            200: inline_serializer(
                name='Success Response',
    ...
    def post(self, request):
        ...
        if 'goods_id' in request.data:
            ...
            return Response({'detail': 'Товар удалён из корзины'},
                            HTTP_204_NO_CONTENT)
        return Response({'detail': 'Необходимо предоставить товар'},
...
class BasketDetailView(GenericAPIView):
    ...
    @extend_schema(description='Для получения детальной информации о корзине,'
