...
class CategoriesBoughByUserView(RetrieveAPIView):
    """
    View для межсервисного взаимодействия,
    используется в store_service в GoodsListView
    """
    ...
    @extend_schema(
        description="Для межсервисного бэкенд взаимодействия, " "не для фронтенд части."
    )
...
class AddPriceExpectation(GenericAPIView):
    ...
    @extend_schema(description="Для добавления товара в ожидаемые при указанной цене")
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            user.save()
            return Response(
                {"detail": "Товар добавлен в ожидаемые"},
                HTTP_200_OK,
            )
        return Response(
            serializer.errors,
...
class RemovePriceExpectation(GenericAPIView):
    ...
    @extend_schema(description="Для удаления товара из ожидаемых")
    def post(self, request, *args, **kwargs):
        serializer = PriceExpectationSerializer(data=request.data)
        if serializer.is_valid():
            ...
            if goods in user.expected_prices:
                ...
                user.save()
                return Response({"detail": "Товар удалён из ожидаемых"}, HTTP_200_OK)
            """
            можно и 404-ый код рассмотреть, но 200 выбрал т.к сама апишка корректно
            отработала
            """
            return Response(
                {"detail": "Товар отсутствует в ожидаемых"},
                status=HTTP_200_OK,
            )
