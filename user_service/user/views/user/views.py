    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            request.session["edit_user_email"] = {
                "full_code": full_code,
                "expire_at": str(
                    datetime.datetime.now() + timezone.timedelta(minutes=5)
                ),
            }
            ...
            return Response(
                {"detail": f"Письмо отправлено на почту. Код: {short_code}"},
                status=status.HTTP_200_OK,
            )
        except ObjectDoesNotExist:
            return Response(
                {"detail": "Неверно указан email."},
...
class ConfirmEditUserEmailView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if check_password(short_code, full_code):
            if request.session["edit_user_email"].get("expire_at") > str(
...
class SendMailNewEmailView(GenericAPIView):
    ...
    @extend_schema(description="Пользователь вводит новую почту.")
    def post(self, request, *args, **kwargs):
        user = get_user_model().objects.get(pk=self.request.user.pk)
        short_code = str(random.randrange(100000, 999999))
        full_code = make_password(short_code)
        ...
        send_mail_task(user_email=user.email, subject="Смена email.", message=f"")
        return Response(
            {"detail": f"Отправлено письмо на почту. Код: {short_code}"},
            status=status.HTTP_200_OK,
        )
...
class SetNewEmailView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if check_password(short_code, full_code):
            if request.session["send_mail_new_email"].get("expire_at") > str(
                datetime.datetime.now()
            ):
                ...
                return Response(
                    {"detail": "Пользователь поменял почту."},
                    status=status.HTTP_200_OK,
                )
            else:
                del request.session["send_mail_new_email"]
...
class EditUserPasswordView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        user.save()
        return Response(
            {"detail": f"У пользователя {user.email} изменен пароль"},
            status=status.HTTP_200_OK,
        )
...
class UserHistoryView(GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        token = {
            "Authorization": f"Q {str(requests.post('http://127.0.0.1:8003/payment/api/login', data).json()['access'])}"
        }
        url = "http://127.0.0.1:8003/payment/order-list/"
        response = requests.get(url, headers=token)
        orders = response.json()
        return Response(
            {"detail": {"user": user.email, "orders_list": orders}},
            status=HTTP_200_OK,
        )
